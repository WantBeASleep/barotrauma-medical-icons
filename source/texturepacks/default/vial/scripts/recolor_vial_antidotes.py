...
VIAL_DIR = PROJECT_ROOT / "source" / "texturepacks" / "default" / "vial"
BASE_DIR = VIAL_DIR / "base"
...
def icon_masks(size: tuple[int, int]) -> dict[str, Image.Image]:
    source = Image.open(BASE_DIR / "icon_source.png").convert("RGBA")
    if size != (64, 64):
...
def save_manual_mask_preview() -> None:
    icon = Image.open(BASE_DIR / "icon.png").convert("RGBA")
    icon_source = Image.open(BASE_DIR / "icon_source.png").convert("RGBA")
    sprite = Image.open(BASE_DIR / "sprite.png").convert("RGBA")
...
def main() -> None:
    base_icon_source = Image.open(BASE_DIR / "icon_source.png").convert("RGBA")
    base_sprite = Image.open(BASE_DIR / "sprite.png").convert("RGBA")
